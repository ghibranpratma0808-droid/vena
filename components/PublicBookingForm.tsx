...
const PublicBookingForm: React.FC<PublicBookingFormProps> = ({ 
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await PublicAPI.submitBooking({
                clientData: {
                ...
                projectData: {
                    ...
                    promoCodeId: discountAmount > 0 && formData.promoCode ? 
                        promoCodes.find(p => p.code === formData.promoCode.toUpperCase().trim())?.id : undefined,
            ...
            if (leadId) {
                setLeads(prev => prev.map(l => 
                    l.id === leadId
                    ? { ...l, status: LeadStatus.CONVERTED, notes: `Dikonversi dari formulir booking. Klien ID: ${result.client.id}` }
                    : l
                ));
            }
            ...
            addNotification({
                ...
                message: `Booking dari ${result.client.name} untuk proyek "${result.project.project_name}" menunggu konfirmasi Anda.`,
                icon: 'lead',
                link: { view: ViewType.BOOKING }
            });
        } catch (error) {
            console.error('Error submitting booking:', error);
            showNotification('Gagal mengirim booking. Silakan coba lagi.');
        } finally {
